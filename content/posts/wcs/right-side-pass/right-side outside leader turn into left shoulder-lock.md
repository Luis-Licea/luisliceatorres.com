---
title: "right-side outside leader turn into left shoulder-lock"
date: 2026-06-15T01:31:24-05:00
description: ""
categories: Dance
tags: West Coast Swing
---

- Start in an {{% a "open" %}} hold. Same as a right-side pass up to step 4.
- On the 5&6, the leader turns clockwise 1 time to the left of the slot into a {{% a "left shoulder-lock" %}}. The back part of the hand should be firmly placed on the follower's right shoulder for leading the following step.

Consider performing the 5&6 as an {{% a "cross-legged outside turn" %}}.

This pattern can be followed by a {{% a "left shoulder-lock reverse whip" %}}.
